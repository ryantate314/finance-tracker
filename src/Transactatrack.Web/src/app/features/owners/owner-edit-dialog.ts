...
export interface OwnerEditDialogData {
  name?: string;
}
...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>{{ data.name ? 'Edit Owner' : 'New Owner' }}</h2>
    <mat-dialog-content>
      <form [formGroup]="form">
...
export class OwnerEditDialog {
  data: OwnerEditDialogData = inject(MAT_DIALOG_DATA);
  private ref = inject(MatDialogRef<OwnerEditDialog>);
